...
//*********Console log chats
			if (!isGroup && isCmd) console.log('\e[1;37m│\e[0m', '\x1b[1;32m>', time, color(command), 'from', color(pushname), 'args :', (args.length))
			
//*********Console log grupos
			if (isCmd && isGroup) console.log('\e[1;37m│\e[0m', '\x1b[1;32m>', time, color(command), 'from', (groupName), 'args :', color(args.length))
...
pushnem = sam.key.fromMe ? samu330.user.name : conts.notify || conts.vname || conts.name || '-'
 ...
 pushnem = num.split('@')[0]
			...
			try {
				...
				exec(`magick './src/reg.jpg' -gravity west -fill '#00FFFF' -font './src/Capture_it.ttf' -size 1280x710 -pointsize 75 -interline-spacing 7.5 -annotate +460-45 '${pushnem}' -pointsize 50 -annotate +460+200 '${serialUser}' '${ppimg}' -resize %[fx:t?u.w*0.2:u.w]x%[fx:?u.h*0.2:u.h] -gravity center -geometry -430+70 -composite 'sam.jpg'`)
				.on('error', () => reply('error'))
				.on('exit', () => {
veri = sender
if (isGroup) {
  addRegisteredUser(sender, namaUser, umurUser, time, serialUser)
			samu330.sendMessage(mdata.id, fs.readFileSync('sam.jpg'), MessageType.image, {caption: `〘  *Vericación* 〙\n\nCódigo : *${serialUser}*\n\n◦ *Nombre* : *${namaUser}*\n\n◦ *Número* : *${sender.split("@")[0]}*`, contextInfo: { mentionedJid: [num] }})              
			})
					...
					await ffmpeg(`${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
									...
									samu330.sendMessagefrom, fs.readFileSync(`./sticker/${sender}.webp`),sticker, {quoted: fdoc, contextInfo: {"forwardingScore": 9999, "isForwarded": true}})
									fs.unlinkSync(media)	
						...
						await ffmpeg(`${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
									...
									samu330.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: fdoc, contextInfo: {"forwardingScore": 9999, "isForwarded": true}})
									fs.unlinkSync(media)
									fs.unlinkSync(`./sticker/${sender}.webp`)
								})
							})
