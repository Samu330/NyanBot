...
samu330.sendMessage(from, buffer, image, {
  ...
  break
			...
			//	if (!isPrem) return reply(mess.prem)
				if (!isQuotedSticker) return reply(`Etiqueta un sticker con el comando *${prefix}robar nomre|autor*`)
				pembawm = args.join(' ')
				if (!pembawm.includes('|')) return reply(`Etiqueta un sticker con el comando *${prefix}robar nomre|autorr*`)
				encmedia = isQuotedSticker ? JSON.parse(JSON.stringify(sam).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : sam
				media = await samu330.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
				...
				atm = args.join(' ')
				args = atm.split('|')
				if (!args) return reply(from, `Forma de uso ${prefix}contacto @tag _*(Para uso en grupos)*_ o...\nnumero|nombre`)
