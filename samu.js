  ...
  break
			...
			case 's':
				if (isMedia && !sam.message.videoMessage || isQuotedImage) {
					...
					await ffmpeg(`${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
									...
									samu330.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`) ,sticker, sam)
									fs.unlinkSync(media)	
					...
					const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(qul).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : sam
					const media = await samu330.downloadAndSaveMediaMessage(encmedia, `./sticker/${sender}`)
					reply(mess.ferr)
						await ffmpeg(`${media}`)
							...
							.on('end', function () {
								...
								exec(`webpmux -set exif ./sticker/data.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
									if (error) return reply(mess.ferr)
									samu330.sendSticker(from, fs.readFileSync(`./sticker/${sender}.webp`))
									fs.unlinkSync(media)
									fs.unlinkSync(`./sticker/${sender}.webp`)
								})
							})
...
case 'antidelete':
		...
		case 'takestick':
                    if ((isMedia && !sam.message.videoMessage || isQuotedSticker)) {
                        ...
                        request({
                            ...
                            fs.writeFileSync(file_name, body, "bin")
		...
		case 'blackpink':
		    ...
		    reply(mess.wait)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/textprome/${command}?apikey=ec35353a991a258b05876861&text=${ini_txt}`).then((gambar) => {
				...
				reply(mess.wait)
                    ...
                    txt1 = a.split("|")[0];
                    txt2 = a.split("|")[1];
                    getBuffer(`https://api.lolhuman.xyz/api/textprome2/${command}?apikey=ec35353a991a258b05876861&text1=${txt1}&text2=${txt2}`).then((gambar) => {
				...
				reply(mess.wait)
                    ini_txt = args.join(' ')
                    getBuffer(`https://api.lolhuman.xyz/api/photooxy1/${command}?apikey=ec35353a991a258b05876861&text=${ini_txt}`).then((gambar) => {
                        samu330.sendMessage(from, gambar, image, { quoted: fgc })
                    })
                    break
                case 'tiktok':
		    ...
		    a = args.join(' ')
                    txt1 = a.split("|")[0];
                    txt2 = a.split("|")[1];
                    getBuffer(`https://api.lolhuman.xyz/api/photooxy2/${command}?apikey=ec35353a991a258b05876861&text1=${txt1}&text2=${txt2}`).then((gambar) => {
                        samu330.sendMessage(from, gambar, image, { quoted: fgc })
                    })
                    break
		...
		case 'google':
                    ...
                    query = args.join(' ')
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/gsearch?apikey=ec35353a991a258b05876861&query=${query}`)
...
case 'play':
  ...
  if (args.length == 0) return reply(`Ejemplo: ${prefix + command} Me olvide de vivir`)
                    query = args.join(' ')
		    fakee = fs.readFileSync('./src/img.jpg')
                    get_result = await fetchJson(`https://api.lolhuman.xyz/api/ytplay?apikey=ec35353a991a258b05876861&query=${query}`)
                    get_result = get_result.result
