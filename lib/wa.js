...
exports.hideTagImage = async(from, buffer) => {
    ...
    await samu330.sendMessage(from, buffer, MessageType.image, { contextInfo: { mentionedJid: members } })
                                                                                                                                                                                 ...
                                                                                                                                                                                 exports.hideTagSticker = async(from, buffer) => {                                                                                                                                    members = await this.getGroupParticipants(from)                                                                                                                                  await samu330.sendMessage(from, buffer, MessageType.sticker, { contextInfo: { mentionedJid: members } })
}
...
exports.getGroupParticipants = async(id) => {
    ...
    let mem = []
    for (let i of members) {                                                                                                                                                             mem.push(i.jid)                                                                                                                                                              }
    return mem                                                                                                                                                                   }
...
exports.demoteAdmin = async(to, target = []) => {
    const g = await samu330.groupMetadata(to)                                                                                                                                        const owner = g.owner.replace("c.us", "s.whatsapp.net")
    const me = samu330.user.jid
    for (i of target) {
        if (!i.includes(me) && !i.includes(owner)) {
